import { getContinueReadingList, clearReadingHistory } from './reading-history.js';
...
async function loadRecentHistory() {
  ...
  try {
    ...
    container.innerHTML = readingList.slice(0, 20).map(item => `
...
function removeFavorite(mangaId, event) {
  ...
  LIBRARY_STATE.favorites = LIBRARY_STATE.favorites.filter(f => f.mangaId !== mangaId);
  saveFavorites();
  loadFavorites();
}

function saveFavorites() {
  localStorage.setItem('enokku_favorites', JSON.stringify(LIBRARY_STATE.favorites));
}

function addBookmark(mangaId, chapterId, chapterNumber, pageNumber, mangaTitle, coverUrl) {
  const bookmark = {
...
function removeBookmark(bookmarkId) {
  LIBRARY_STATE.bookmarks = LIBRARY_STATE.bookmarks.filter(b => b.id !== bookmarkId);
  saveBookmarks();
  loadBookmarks();
}

function saveBookmarks() {
  localStorage.setItem('enokku_bookmarks', JSON.stringify(LIBRARY_STATE.bookmarks));
}
