...
if (!chapterId) {
    readerImages.innerHTML = '<div class="error"><p>No chapter ID provided</p></div>';
}
...
async function loadChapter() {
    ...
    try {
        ...
        if (!pages || pages.length === 0) {
            readerImages.innerHTML = '<div class="error"><p>No pages available for this chapter</p></div>';
            return;
        }
...
function renderPages() {
    ...
    pages.forEach((page, index) => {
        ...
        img.onerror = () => {
            img.src = 'https://via.placeholder.com/800x1200?text=Failed+to+load';
        };
